  ...
  return (
    <section className="min-h-dvh py-8 px-4 flex flex-col items-center">
      ...
      >
        ...
        <h1 className="font-heading text-4xl sm:text-5xl gradient-text mb-1">WhatsApp</h1>
      ...
      <div className="w-full max-w-md">
        ...
        >
          ...
          {/* Pinned Message */}
          <div className="px-3 py-2 flex items-center gap-2 border-b border-white/5 shrink-0 relative z-10" style={{ background: '#192028' }}>
          ...
          {/* Month Selector */}
          <div className="shrink-0 bg-[#0b141a] px-3 py-2 border-b border-white/5 flex gap-2 overflow-x-auto relative z-10" style={{ scrollbarWidth: 'none' }}>
            {monthsMap.map(([label, groupIndex]) => (
              <button
                key={label}
                onClick={() => virtuosoRef.current?.scrollToIndex({ index: groupIndex, align: 'start', behavior: 'smooth' })}
              ...
              <GroupedVirtuoso
                ...
                groupContent={(index) => (
                  <div className="flex justify-center py-4">
                    <span className="px-3 py-1 rounded-full text-[10px] font-sans shadow-md" style={{ background: '#1f2c34', color: 'rgba(255,255,255,0.6)' }}>
                      {groupLabels[index]}
                    </span>
                  </div>
                )}
